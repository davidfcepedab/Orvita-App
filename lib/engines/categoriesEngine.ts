...
/**
 * Builds a structural tree from transactions
 */
export function buildStructuralTree(
  ...
  financialCategories: { name: string; total: number }[]
  totalFixed: number
  totalVariable: number
  totalStructural: number
  totalFinancialFlow: number
} {
  ...
  transactions.forEach((tx) => {
    ...
    if (EXCLUDED.includes(categoria as typeof EXCLUDED[number])) {
      if (mes === month) {
  ...
  const structuralCategories = Object.entries(currentMap)
    .map(([name, data]) => {
      ...
      const subs: Subcategory[] = Object.entries(data.subcategories).map(
        ([subName, value]) => ({
          name: subName,
          total: value,
        })
      ...
      return {
        ...
        delta,
        type: (
          FIXED_CATEGORIES.includes(name as typeof FIXED_CATEGORIES[number])
            ? "fixed"
            : "variable"
        ) as "fixed" | "variable",
        subcategories: subs,
